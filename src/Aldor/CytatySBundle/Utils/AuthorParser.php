...
class AuthorParser
{
    ...
    public  function getAuthorAndText()
    {
        ...
        if($separationChar == '[')
        {
            $endSerparatorPos = mb_strpos($author,']',0,$this->encoding);
            $author = mb_substr($author,0,$endSerparatorPos,$this->encoding);
        }
        else if($separationChar == '~' && mb_strpos($author,']',0,$this->encoding))
        {
            $tmp = explode('[',$author);//,$this->encoding);
            $author = $tmp[0];
             $endSerparatorPos = mb_strpos($tmp[1],']',0,$this->encoding);
             $additionalinfo =   mb_substr($tmp[1],0,$endSerparatorPos,$this->encoding);


        }
        return array('author' => $author,'text' => $text,'additionalinfo' => $additionalinfo);
    }
